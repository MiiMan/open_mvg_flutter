...
Java_com_example_open_1mvg_1flutter_1java_bridge_OpenMVG_imageList(
        JNIEnv *env,
        jobject obj, jstring sImageDirS, jstring sfileDatabaseS, jstring sOutputDirS,
        jstring jfocal_pixels) {

    std::string failRes = "1";
    std::string failRes1 = "1";
    std::string failRes2 = "2";
    std::string failRes3 = "3";
    std::string failRes4 = "4";
    std::string failRes5 = "5";
    std::string failRes6 = "6";
    std::string failRes7 = "7";
    std::string failRes8 = "8";
    std::string failRes9 = "9";
    ...
    if (!stlplus::folder_exists(sImageDir)) {
        ...
        return env->NewStringUTF(failRes9.c_str());
    }

    if (sOutputDir.empty()) {
        std::cerr << "\nInvalid output directory" << std::endl;
        return env->NewStringUTF(failRes2.c_str());
    }

    if (!stlplus::folder_exists(sOutputDir)) {
        if (!stlplus::folder_create(sOutputDir)) {
            std::cerr << "\nCannot create output directory" << std::endl;
            return env->NewStringUTF(failRes3.c_str());
        }
    }
    ...
    if (sKmatrix.size() > 0 &&
        ...
        return env->NewStringUTF(failRes4.c_str());
    }

    if (sKmatrix.size() > 0 && focal_pixels != -1.0) {
        std::cerr << "\nCannot combine -f and -k options" << std::endl;
        return env->NewStringUTF(failRes5.c_str());
    }
            ...
            switch (e_User_camera_model) {
                ...
                default:
                    std::cerr << "Error: unknown camera model: " << (int) e_User_camera_model
                              << std::endl;
                    return env->NewStringUTF(failRes7.c_str());
            }
        }
