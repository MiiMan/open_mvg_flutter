...
Java_com_example_open_1mvg_1flutter_1java_bridge_OpenMVG_incrementalReconstruct(
        JNIEnv *env,
    ...
    if (intrinsic_refinement_options == static_cast<cameras::Intrinsic_Parameter_Type>(0)) {
        std::cerr << "Invalid input for the Bundle Adjusment Intrinsic parameter refinement option"
                  << std::endl;
        return env->NewStringUTF("FAILED3");
    }
    ...
    if (!StringToEnum_ESfMSceneInitializer(sSfMInitializer_method, scene_initializer_enum)) {
        ...
        return env->NewStringUTF("FAILED4");
    }
    ...
    if (!Load(sfm_data, sSfM_Data_Filename, ESfM_Data(VIEWS | INTRINSICS | EXTRINSICS))) {
        std::cerr << std::endl
                  ...
                  << std::endl;
        return env->NewStringUTF("FAILED5");
    }
    ...
    if (!regions_type) {
        ...
        return env->NewStringUTF("FAILED6");
    }
    ...
    if (!feats_provider->load(sfm_data, sMatchesDir, regions_type)) {
        ...
        return env->NewStringUTF("FAILED7");
    }
    // Matches reading
    ...
    if // Try to read the provided match filename or the default one (matches.f.txt/bin)
        ...
        return env->NewStringUTF("FAILED8");
    }

    if (sOutDir.empty()) {
        std::cerr << "\nIt is an invalid output directory" << std::endl;
        return env->NewStringUTF("FAILED9");
    }
    ...
    switch (scene_initializer_enum) {
        case ESfMSceneInitializer::INITIALIZE_AUTO_PAIR:
            ...
            return env->NewStringUTF("FAILED10");
            break;
        ...
        case ESfMSceneInitializer::INITIALIZE_STELLAR:
            ...
            break;
        default:
            return env->NewStringUTF("FAILED11");
    }
    if (!scene_initializer) {
        std::cerr << "Invalid scene initializer." << std::endl;
        return env->NewStringUTF("FAILED12");
    }
    ...
    return env->NewStringUTF("FAILED13");
}
